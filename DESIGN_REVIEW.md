# Design review dump (API key + DB schema)

This document aggregates the current API key workflow/integrity rules and the PostgreSQL migration snippet so you can review them together.
...
- Each logbook has its own API key. Requests must present that API key. The logging station `station_callsign` field must match the logbook’s callsign.
...
2) Server generates an API key for that logbook:
   ...
   - Return full key = `prefix.secretHex` to the client over TLS (only once, but what happens if the internet connection drops?)
...
4) When the client pushes QSOs to the server, every request includes:
   ...
   - Each QSO is linked with `logbook_id`. If QSO also carries a logging station callsign, it must equal the logbook’s callsign.
     ...
     The client will generally push a single QSO to the server for each request; qsos are generally logged locally and then pushed to the server real time.
5) Server verification (per request):
   ...
   - On QSO writes, enforce that the logging station_callsign equals the logbook’s callsign (the field is madatory); the contacted station callsign (`qso.call`) is unconstrained.
...
- With 32 bytes of random secret, a fast cryptographic hash (SHA‑512) is enough to store server-side digests.
...
## PostgreSQL schema (migration snippet)
